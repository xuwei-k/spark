...
private[streaming] class JavaStreamingListenerWrapper(javaStreamingListener: JavaStreamingListener)
  ...
  private def toJavaBatchInfo(batchInfo: BatchInfo): JavaBatchInfo = {
    JavaBatchInfo(
      batchInfo.batchTime,
      batchInfo.streamIdToInputInfo.mapValues(toJavaStreamInputInfo(_)).toMap.asJava,
      ...
      batchInfo.outputOperationInfos.mapValues(toJavaOutputOperationInfo(_)).toMap.asJava
    )
  }
